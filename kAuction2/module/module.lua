...
function kAuction2:NewModuleType(name, defaults, update_texts)
	if DEBUG then
...
local Module = {}
kAuction2:SetDefaultModulePrototype(Module)

local do_nothing = function() end

--- Iterate through all script hooks for a given script
-- @param script name of the script
-- @usage for module, func in kAuction2:IterateFrameScriptHooks("OnEnter") do
--     -- do stuff here
-- end
-- @return iterator that returns module and function
function kAuction2:IterateFrameScriptHooks(script)
	if DEBUG then
...
-- @usage kAuction2:RunFrameScriptHooks(script, ...)
function kAuction2:RunFrameScriptHooks(script, frame, ...)
	if DEBUG then
		expect(script, 'typeof', 'string')
		expect(frame, 'typeof', 'frame')
		expect(frame, 'inset', kAuction2.all_frames)
	end
...
function kAuction2:OnModuleCreated(module)
...
LibStub("AceEvent-3.0"):RegisterEvent("ADDON_LOADED", function(event, addon)
	if not kAuction2.Options or not kAuction2.Options.HandleModuleLoad then
		return
	end
	while true do
...
function Module:AddFrameScriptHook(script, method)
	...
	module_script_hooks[script][self] = kAuction2.Utils.ConvertMethodToFunction(self, method)
end
...
function Module:UpdateForUnitID(unit)
	...
	for frame in kAuction2:IterateFramesForUnitID(unit) do
		self:Update(frame)
	end
end
...
function Module:UpdateForGUID(guid)
	...
	for frame in kAuction2:IterateFramesForGUID(guid) do
		self:Update(frame)
	end
end
...
function Module:UpdateForClassification(classification)
	...
	for frame in kAuction2:IterateFramesForClassification(classification) do
		self:Update(frame)
	end
end
...
function Module:UpdateAll()
	for frame in kAuction2:IterateFrames() do
		self:Update(frame)
	end
end
...
function Module:UpdateNonWacky()
	for frame in kAuction2:IterateNonWackyFrames() do
		self:Update(frame)
	end
	for frame in kAuction2:IterateWackyFrames() do
...
-- @usage for id, module in kAuction2:IterateEnabledModules() do
--     doSomethingWith(module)
-- end
-- @return iterator which returns the id and module
function kAuction2:IterateEnabledModules()
...
local function iter(types, id)
	local id, module = next(kAuction2.modules, id)
	if not id then
		-- we're out of modules
		del(types)
		return nil
...
-- @usage for id, module in kAuction2:IterateModulesOfType("bar") do
--     doSomethingWith(module)
-- end
-- @usage for id, module in kAuction2:IterateModulesOfType("bar", "indicator", true) do
--     doSomethingWith(module)
-- end
-- @return iterator which returns the id and module
function kAuction2:IterateModulesOfType(...)
	local types = new()
	local n = select('#', ...)
...
do
	...
	local old_kAuction2_OnInitialize = kAuction2.OnInitialize
	kAuction2.OnInitialize = function(self)
		if old_kAuction2_OnInitialize then
			old_kAuction2_OnInitialize(self)
		end
...
-- @usage kAuction2:EnableModule(MyModule)
function kAuction2:EnableModule(module)
...
-- @usage kAuction2:DisableModule(MyModule)
function kAuction2:DisableModule(module)
	if not module:IsEnabled() then
		return
	end
